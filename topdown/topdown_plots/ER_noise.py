...
def plot_er_graph(data, cand, race, elect, elect_col, tot_vote, eps, split, filt=True, filt_thresh=10, n_samps=32, 
                  ax=None, title=True, plot_cvap=False, weight=False):
    
    df = data.query("epsilon == @eps & split == @split")
    df = df.query("`{}` > {}".format(tot_vote, filt_thresh)) if filt else df
...
def plot_elect_grid(epsilon_values, epsilon_splits, data, candidate, race, elect_col, 
                    tot_vote, n_samps=32, figsize=(10,10), filt=True, filt_thresh=10, title=None, weight=False):
    ...
    for i in range(len(epsilon_values)):
        for j in range(len(epsilon_splits)):
            plot_er_graph(data, candidate, race, None, elect_col, tot_vote, 
                          ...
                          title=False, ax=axs[i,j], filt=filt, filt_thresh=filt_thresh, weight=weight, n_samps=n_samps)
...
def point_estimates(data, cand, race, elect_col, tot_vote, eps, split, filt=True):
