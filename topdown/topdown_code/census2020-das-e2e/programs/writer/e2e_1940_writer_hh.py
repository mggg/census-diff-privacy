...
# Number of partitions for writing out text file
NUM_PARTITIONS = 100
...
def to_per_line(record):
    ...
    # bhushan's additions to not care about the hhgq fields
    euid = 1
    epnum = 1
    hhgq = 1
    ...
    #assert len(geocode) == 10, "Geocode of record {} has length {} instead of 16".format(record, len(geocode))
    STATE = geocode[:2]
    COUNTY = geocode[2:5]
    ENUMDIST = geocode[5:]
    ...
    QRACE8 = "9999"
    CIT = "9"
    line = [SCHEMA_TYPE_CODE, SCHEMA_BUILD_ID, STATE, COUNTY, ENUMDIST,
...
def to_unit_line(record):
    ...
    if int(hhgq) in [1,2,3]:
        hhgq = hhgq + 1
    elif int(hhgq) in [4,5,6,7]:
        hhgq = hhgq + 2
    else:
        hhgq = 0
...
def to_per_input(row):
    ...
    geo, _, _, age, hisp, cenrace = per_tuple
    #_, global_id, _ = unit_tuple
    return (geo, _, _, _, age, hisp, cenrace) 
...
class writer(AbstractDASWriter):
    ...
    def write(self, input_tuple):
        ...
        per_data = block_nodes.map(lambda node: ((node.geocode,), (node.syn,)))
        ...
        per_data = per_data.map(to_per_input).persist()
        
        per_output = per_data.map(to_per_line).persist()
        print("Wrote {} people: ", per_output.count())
        ...
        #unit_output.coalesce(NUM_PARTITIONS).saveAsTextFile(unit_path)
        per_output.coalesce(NUM_PARTITIONS).saveAsTextFile(per_path)
        ...
        def dump_metadata( path, now, count):
            ...
            if Path(path).parent.is_dir():
                ...
                print("Metadata saving to local output location")
                f = open(path, "w")
            else:
                # Save metadata remotely
                print("Metadata saving to S3 output location")
                f = ctools.s3.s3open(path, "w")
            
            f.write("# UUI --- This file is based on 1940 public data. Distribution Unlimited.\n")
            f.write("# Created: {}\n".format( now))
            f.write("# Records: {}\n".format( count))
