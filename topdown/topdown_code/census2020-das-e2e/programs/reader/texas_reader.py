...
class reader(AbstractDASReader):
    ...
    def load_ipums_1940(self):
        ...
        import programs.reader.ipums_1940_rdd_blocks as ipums_1940_rdd
    ...
    def read(self):
        ...
        print("Bhushan:: Printing Person Data")
        print(per_data.first())
        print("Num persons: {}".format(per_data.count()))
        print()
        print("Bhushan: Printing Unit Data")
        print(unit_data.first())
        print("Num units: {}".format(unit_data.count()))
        print()

        join_data = self.join_hist_agg(per_data, unit_data)
        
        print("Bhushan: Join data")
        print(join_data.first())
        print()
        
        block_nodes = join_data.map(self.make_block_node).coalesce(BLOCK_NODE_PARTITIONS).persist()

        print("Bhushan: printing Block nodes")
        print(block_nodes.first())
        print("Num blocks: {}".format(block_nodes.count()))
        print()
        # join_data.unpersist()
        return block_nodes
    ...
    def make_block_node(self, person_unit_arrays):
        ...
        if self.config.has_option(CONSTRAINTS, THE_INVARIANTS+"."+levels[0]):
        ...
        if self.config.has_option(CONSTRAINTS, THE_CONSTRAINTS+"."+levels[0]):
