...
class engine(AbstractDASEngine):
    ...
    def noisyAnswers(self, nodes_dict):
        ...
        for level in levels:
            ...
            print("Bhushan is in the engine! at level: " + level)
    ...
    def getNodesAllGeolevels(self, block_nodes):
        ...
        """
        print("Inside getNodesAllGeoLevels")
        print()
        config = self.config
...
def make_dp_node(config, geounitNode):
    ...
    if config.has_option(BUDGET,'DPqueries'):
...
def makeAdditionalInvariantsConstraints(node, config):
    ...
    if config.has_option(CONSTRAINTS, THEINVARIANTS+"."+level):
        ...
        if node.invar is None:
            node.invar = invariants_dict
        else:
    ...
    if config.has_option(CONSTRAINTS, THECONSTRAINTS+"."+level):
        ...
        print(level)
        print("Inside that pesky function")
        if node.cons is None:
            node.cons = constraints_dict
        else:
            node.cons.update(constraints_dict)
    return node

def add_empty_cons_dict_to_node(node):
    node.cons = {}
    return node
